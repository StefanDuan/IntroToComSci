...
def postRetirement(savings, growthRates, expenses):
    ...
    for growthRate in growthRates[1:]:
        i += 1
        F.append(F[i-1]*(1 + 0.01*growthRate) - expenses)
    return F
...
def findMaxExpenses(salary, save, preRetireGrowthRates, postRetireGrowthRates,
    ...
    low = savings[-1]/len(postRetireGrowthRates)
    # low = 0
    high = savings[-1]
    currentExpence = (low + high)/2.0
    print 'Current estimation of expence: ', currentExpence
    fund = postRetirement(savings[-1], postRetireGrowthRates, currentExpence)
    balance = fund[-1]
